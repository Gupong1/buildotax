...
class _BugSenderPageState extends State<BugSenderPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _addSender(String number, String name) async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
            "http://panelprivatkenzxcpanelpanel.pterodactylxcz.my.id:4113/getPairing?key=${widget.sessionKey}&number=$number"),
      );
    ...
    if (confirmed == true) {
      ...
      try {
        final response = await http.delete(
          Uri.parse(
              "http://panelprivatkenzxcpanelpanel.pterodactylxcz.my.id:4113/deleteSender?key=${widget.sessionKey}&id=$senderId"),
        );
